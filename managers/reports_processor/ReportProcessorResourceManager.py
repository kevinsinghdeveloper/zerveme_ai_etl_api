...
class ReportProcessorResourceManager(IResourceManager):
    def __init__(
            self,
            web_service_managers: Optional[
                Dict[str, IServiceManagerBase]] = None,
            **kwargs):
        super().__init__(web_service_manager=web_service_managers)
        self.__etl_service_manager = web_service_managers.get(
            "etl_service_manager")

    def get(
            self,
            request_resource_model: ReportProcessorRequestResourceModel):
        return jsonify({"message": "Getting status", "data": []})

    def post(
            self, request_resource_model: ReportProcessorRequestResourceModel):
        # TODO based on the request get the corresponding AI service manager

        llm_manager = AIServiceHandler.get_ai_service(
            request_resource_model.llm_config)
        self.__etl_service_manager.configure(llm_manager=llm_manager)

        # TODO configure should be linking -- we need to pass some test LLM
        # credentials and test
        # TODO what else do we need to do? LLM should keep history no?
        # or we should be ok

        self.__etl_service_manager.run_task(request_resource_model)
