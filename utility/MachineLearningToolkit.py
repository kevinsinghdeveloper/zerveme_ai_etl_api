...
class MachineLearningToolkit:
    ...
    def get_product_jaccard_sim(
        ...
        og_product_tokens = MachineLearningToolkit.tokenize_title(product_resource.product_title)

        # Apply Jaccard similarity for each row in the DataFrame
        related_products_df['similarity'] = related_products_df[related_product_name_field].apply(
    ...
    def get_product_cosine_sim(
        ...
        product_titles = [product_resource.product_title] + related_products_df[related_product_name_field].tolist()

        # Compute TF-IDF vectors
        vectorizer = TfidfVectorizer()
        tfidf_matrix = vectorizer.fit_transform(product_titles)

        # Calculate cosine similarities between the original product and related products
    ...
    def get_product_sbert_sim(
        ...
        product_embedding = self.__model.encode(product_title, convert_to_tensor=True)

        # Get embeddings for all related product titles
        related_titles = related_products_df[related_product_name_field].tolist()
        related_embeddings = self.__model.encode(related_titles, convert_to_tensor=True)

        # Compute cosine similarity using SBERT embeddings
        sbert_similarities = util.cos_sim(product_embedding, related_embeddings).flatten().tolist()
